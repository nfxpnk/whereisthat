...
- `disks.disk_type` is constrained to `CD`, `DVD`, `BluRay`, `HardDisk`, `SolidStateDisk`, `RemovableUSB`, `VirtualDisk`, or `Other`. Mounted ISO-file sources are stored as `VirtualDisk`, Windows CD/DVD drive volumes as `CD`, Windows removable drives as `RemovableUSB`, and fixed drives as `HardDisk` or `SolidStateDisk` when native seek-penalty metadata reliably reports the distinction; sources without a reliable subtype remain `Other`.
