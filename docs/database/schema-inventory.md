...
| `disk_scan_statistics` | `disk_id`, `last_scanned_at`, `image_scanning_time_ms`, `imported_descriptions_count`, `calculated_file_crcs` | [schema/tables/disk_scan_statistics.sql](schema/tables/disk_scan_statistics.sql) | [tables/disk_scan_statistics.md](tables/disk_scan_statistics.md) |
| `folders` | `id`, `disk_id`, `parent_folder_id`, `path`, `name`, `created_at`, `modified_at`, `accessed_at`, `attributes`, `content_size` | [schema/tables/folders.sql](schema/tables/folders.sql) | [tables/folders.md](tables/folders.md) |
...
| Migrations | None. Former catalog formats and normalized catalogs missing `folders.content_size` are rejected without alteration. |
...
| File/folder capture | `FileScanner::ScanFolder`, `Database::InsertFolder`, `UpdateFolderContentSize`, `InsertFile` | `folders`, `files`, including scan-time folder totals. |
