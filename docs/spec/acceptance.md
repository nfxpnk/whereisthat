...
- New or replacement scans preserve `VirtualDisk` and `RemovableUSB` classification and persist `HardDisk` or `SolidStateDisk` for fixed volumes only when the native Windows seek-penalty query reports that distinction reliably; otherwise they retain `Other`.
