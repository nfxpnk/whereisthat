...
- On startup the application shows an alpha-stage warning that the app may crash, lose unsaved changes, or produce incomplete or incorrect catalog data, and that users should keep backups and use it at their own risk. The warning can be bypassed from automation or trusted shortcuts by passing `--bypass-alpha-warning`, for example `WhereIsThat.exe "C:\Catalogs\media.db" --bypass-alpha-warning`, or persistently disabled with the User Interface setting `Do not show alpha warning on startup`.
...
- General Settings opens a dedicated WTL/ATL-hosted native settings dialog in `src/app/wit_gui`; its initial `Show Status Bar` and `Show Toolbar` preferences immediately control status-bar and toolbar visibility and default to enabled. The User Interface page also exposes `Do not show alpha warning on startup`, defaulting to disabled. It lets the user choose a numeric 24-hour date-time display pattern or use the Windows default date/time settings, and it displays the stored main tree/list splitter position and last-opened catalog path as read-only information.
...
New user-facing views or controls should extend the existing native desktop design and module ownership: presentation and frame orchestration in `src/app/wit_gui`, reusable Win32 helpers in `src/app/wit_win32`, catalog browse/write queries in `src/modules/wit_database`, and search queries in `src/modules/wit_search`.
