...
- General Settings opens a dedicated WTL/ATL-hosted native settings dialog in `src/ui`; its initial `Show status bar` preference immediately controls status-bar visibility and defaults to enabled, and it displays the stored main tree/list splitter position and last-opened catalog path as read-only information.
