...
- Surface the stored last-opened path in General Settings and offer the ten most recent successfully activated catalogs from the File menu.
...
- Use a native save-file dialog for New Catalog and a native open-file dialog for Open, filtering for SQLite/database files while allowing ordinary filesystem paths. New Catalog initializes a fresh empty schema at a new path or a closed existing path after the native overwrite confirmation; if that path identifies any catalog currently open in the application, it reports that conflict and leaves the open catalog untouched. Alternative considered: refusing every existing path, rejected because the standard Save workflow already lets a user deliberately replace a closed file while open catalog sessions still need stronger protection.
...
- Keep a most-recently-used list of up to ten catalog paths in the settings helper, deduplicated case-insensitively and updated only after successful New/Open/Open Recent activation. Populate `File > Open Recent` dynamically and route each item through normal validated activation. General Settings presents `LastCatalogPath` read-only because it is lifecycle state rather than an editable preference.
...
- Replacing a path that is currently represented by an open catalog could invalidate loaded state -> reject any New destination already open in the application, while permitting native-confirmed replacement only for closed paths.
