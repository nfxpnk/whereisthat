...
HBITMAP LoadSearchPngBitmap(IWICImagingFactory* factory, UINT resourceId) {
    ...
    if (SUCCEEDED(factory->CreateDecoderFromStream(stream, nullptr, WICDecodeMetadataCacheOnLoad, &decoder)) &&
        ...
        info.bmiHeader.biWidth = kSearchIconSize;
        info.bmiHeader.biHeight = -kSearchIconSize;
        info.bmiHeader.biPlanes = 1;
        info.bmiHeader.biBitCount = 32;
        info.bmiHeader.biCompression = BI_RGB;
        bitmap = CreateDIBSection(nullptr, &info, DIB_RGB_COLORS, &pixels, nullptr, 0);
        if (!bitmap || FAILED(converter->CopyPixels(nullptr, kSearchIconSize * 4,
            kSearchIconSize * kSearchIconSize * 4, static_cast<BYTE*>(pixels)))) {
...
HIMAGELIST CreateSearchImageList() {
    const HRESULT comResult = CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED);
    const bool uninitializeCom = SUCCEEDED(comResult);
    auto images = ImageList_Create(kSearchIconSize, kSearchIconSize, ILC_COLOR32, 105, 0);
    if (!images) {
    ...
    const auto addImage = [&](UINT id) {
        const auto bitmap = LoadSearchPngBitmap(factory, id);
...
bool SearchExtensionEquals(std::wstring_view extension, std::wstring_view candidate) {
    return std::ranges::equal(extension, candidate, [](wchar_t left, wchar_t right) {
        return std::towlower(left) == std::towlower(right);
    });
}

int SearchImageForFileExtension(std::wstring_view extension) {
...
int SearchImageForEntry(const wit::core::FileEntry& entry) {
    if (entry.isDirectory) return entry.isArchive ? BrowserArchiveImage : BrowserFolderImage;
    const int image = SearchImageForFileExtension(entry.extension);
    if (image != I_IMAGENONE) return image;
    return IsSearchArchiveExtension(entry.extension) ? BrowserArchiveImage : BrowserDocumentImage;
}
...
LRESULT SearchDialog::OnGetDisplayInfo(int, LPNMHDR header, BOOL&) {
    ...
    if (displayInfo->item.mask & LVIF_IMAGE) {
        ...
        displayInfo->item.iImage = entry ? SearchImageForEntry(*entry) : I_IMAGENONE;
    }
...
void SearchDialog::Initialize() {
    ...
    if (const auto images = CreateSearchImageList()) {
        ListView_SetImageList(results_, images, LVSIL_SMALL);
    }
