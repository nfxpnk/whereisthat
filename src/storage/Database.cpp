...
bool Database::InitializeSchema(){
    ...
    Exec("CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY AUTOINCREMENT,catalog_id INTEGER NOT NULL,parent_path TEXT NOT NULL,name TEXT NOT NULL,extension TEXT NOT NULL,size INTEGER NOT NULL,modified_at TEXT NOT NULL,attributes INTEGER NOT NULL DEFAULT 0,FOREIGN KEY (catalog_id) REFERENCES catalogs(id) ON DELETE CASCADE);") &&
...
bool Database::InsertFile(const wit::core::FileEntry& f){ SQLiteStatement s(db_,"INSERT INTO files(catalog_id,parent_path,name,extension,size,modified_at,attributes) VALUES(?,?,?,?,?,?,?);"); s.BindInt64(1,f.catalogId); s.BindText(2,wit::platform::ToUtf8(f.parentPath)); s.BindText(3,wit::platform::ToUtf8(f.name)); s.BindText(4,wit::platform::ToUtf8(f.extension)); s.BindInt64(5,(long long)f.size); s.BindText(6,wit::platform::ToUtf8(f.modifiedAt)); s.BindInt64(7,f.attributes); return sqlite3_step(s.Raw())==SQLITE_DONE; }
